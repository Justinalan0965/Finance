...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol,shares, price) VALUES (?, ?, ?, ?)",user_id, symbol, shares, float(price))
        flash(f"The {shares} shares of {symbol} is bought for the price of {usd(purchase)}")
        return redirect("/")
...
def login():
    ...
    if request.method == "POST":
        ...
        flash(f'You were successfully logged in!')
        return redirect("/")
...
def register():
    ...
    if request.method == "POST":
        ...
        valid_username = db.execute("SELECT * FROM users WHERE username = ?",request.form.get("username"))

        if not request.form.get("username"):
            return apology("Enter the username",400)

        elif not request.form.get("password"):
            return apology("create a password",400)
        ...
        for stock in stocks:
            ...
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    quote = lookup(symbol)
                    if quote is None:
                        return apology("Symbol not found1")
                    price = quote["price"]
                    total_sale = shares * price

                    db.execute("UPDATE users SET cash = cash + :total_sale WHERE id = :user_id",total_sale=total_sale,user_id=session["user_id"])

                    db.execute("INSERT INTO transactions (user_id,symbol,shares,price) VALUES (:user_id,:symbol,:shares,:price)",user_id=session["user_id"],symbol=symbol,shares=shares*(-1),price=price)

                    flash(f"Sold {shares} shares of {symbol} for {usd(total_sale)}!")

                    return redirect("/")

        return apology("symbol not found2")
